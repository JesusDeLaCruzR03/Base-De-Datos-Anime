 ...
 */
public class ControladorInterfazAnime {
    ...
    /* La tabla. */
    @FXML private TableView<Anime> tabla;

    /* La ventana. */
    private Stage escenario;
    /* El modelo de la selección. */
    private TableView.TableViewSelectionModel<Anime> modeloSeleccion;
    /* La selección. */
    private ObservableList<TablePosition> seleccion;
    /* Los renglones en la tabla. */
    private ObservableList<Anime> renglones;

    /* La base de datos. */
    private BaseDeDatosAnimes bdd;
    /* La conexión del cliente. */
    private Conexion<Anime> conexion;
    ...
    @FXML private void agregaAnime(ActionEvent evento) {
        DialogoEditaAnime dialogo;
        try {
            dialogo = new DialogoEditaAnime(escenario, null);
        } catch (IOException ioe) {
            String mensaje = ("Ocurrió un error al tratar de " +
                              "cargar el diálogo de anime.");
        ...
        if (!dialogo.isAceptado())
            return;
        bdd.agregaRegistro(dialogo.getAnime());
        try {
            conexion.enviaMensaje(Mensaje.REGISTRO_AGREGADO);
            conexion.enviaRegistro(dialogo.getAnime());
        } catch (IOException ioe) {
            dialogoError("Error con el servidor",
                         "No se pudo enviar un anime a agregar.");
        }
    }

    /* Edita un estudiante. */
    @FXML private void editaAnime(ActionEvent evento) {
        Anime anime = renglones.get(seleccion.get(0).getRow());
        DialogoEditaAnime dialogo;
        try {
            dialogo = new DialogoEditaAnime(escenario, anime);
        } catch (IOException ioe) {
            String mensaje = ("Ocurrió un error al tratar de " +
                              "cargar el diálogo del anime.");
            dialogoError("Error al cargar interfaz", mensaje);
            return;
        }
        dialogo.showAndWait();
        ...
        try {
            ...
            conexion.enviaRegistro(anime);
            conexion.enviaRegistro(dialogo.getAnime());
        } catch (IOException ioe) {
            dialogoError("Error con el servidor",
                         "No se pudieron enviar animes a modificar.");
        }
        bdd.modificaRegistro(anime, dialogo.getAnime());
    }

    /* Elimina uno o varios estudiantes. */
    @FXML private void eliminaAnime(ActionEvent evento) {
        int s = seleccion.size();
        String titulo = (s > 1) ?
            "Eliminar animes" : "Eliminar anime";
        String mensaje = (s > 1) ?
            "Esto eliminará a los animes seleccionados" :
            "Esto eliminará al anime seleccionado";
        String aceptar = titulo;
        String cancelar = (s > 1) ?
            "Conservar animes" : "Conservar anime";
        if (!dialogoDeConfirmacion(titulo, mensaje, "¿Está seguro?",
                                   aceptar, cancelar))
            return;
        Lista<Anime> aEliminar = new Lista<Anime>();
        for (TablePosition tp : seleccion)
            aEliminar.agregaFinal(renglones.get(tp.getRow()));
        modeloSeleccion.clearSelection();
        for (Anime anime : aEliminar) {
            bdd.eliminaRegistro(anime);
            try {
                conexion.enviaMensaje(Mensaje.REGISTRO_ELIMINADO);
                conexion.enviaRegistro(anime);
            } catch (IOException ioe) {
                dialogoError("Error con el servidor",
                             "No se pudo enviar un anime a eliminar.");
                return;
            }
        }
    }
    ...
    /* Busca estudiantes. */
    @FXML private void buscaAnime(ActionEvent evento) {
        DialogoBuscaAnime dialogo;
        try {
            dialogo = new DialogoBuscaAnime(escenario);
        } catch (IOException ioe) {
            ioe.printStackTrace();
            String mensaje = ("Ocurrió un error al tratar de " +
        ...
        Lista<Anime> resultados =
            bdd.buscaRegistros(dialogo.getCampo(),
                               dialogo.getValor());

        modeloSeleccion.clearSelection();
        for (Anime anime : resultados)
            modeloSeleccion.select(anime);
    }
    ...
    @FXML private void acercaDe(ActionEvent evento) {
        ...
        dialogo.setTitle("Acerca de Administrador de Animes");
        dialogo.setHeaderText(null);
        dialogo.setContentText("Aplicación para administrar animes.\n"  +
    ...
    private void conectar() {
        ...
        try {
            ...
            conexion = new Conexion<Anime>(bdd, enchufe);
    ...
    private void eventoBaseDeDatos(EventoBaseDeDatos evento,
        ...
        case REGISTRO_AGREGADO:
            Platform.runLater(() -> agregaAnime(anime1));
            break;
        case REGISTRO_ELIMINADO:
            Platform.runLater(() -> eliminaAnime(anime1));
            break;
    ...
    private void mensajeRecibido(Conexion<Anime> conexion, Mensaje mensaje) {
        switch (mensaje) {
        case BASE_DE_DATOS:
    ...
    private void baseDeDatos(Conexion<Anime> conexion) {
        try {
    ...
    private void registroAlterado(Conexion<Anime> conexion,
                                  Mensaje mensaje) {
        Anime a;
        try {
    ...
    private void registroModificado(Conexion<Anime> conexion) {
        Anime a1, a2;
        try {
    ...
    private void agregaAnime(Anime anime) {
        renglones.add(anime);
        tabla.sort();
    }

    /* Elimina un estudiante de la tabla. */
    private void eliminaAnime(Anime anime) {
        renglones.remove(anime);
        tabla.sort();
    }
}
